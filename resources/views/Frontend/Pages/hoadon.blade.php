...
<table width="700">
    ...
    <tr>
        <td colspan="8" style="border-top: 4px double #ccc;;font-size:21px; font-weight:bold; text-align:center; padding:15px 0;">BẢNG HÓA ĐƠN SẢN PHẨM</td>
    </tr>
</tbody></table>
  ...
  <table width="700">
    <tbody><tr>
      ...
      <td colspan="3" align="right">
        Ngày tạo: <span id="price_time">{{date('d-m-Y', strtotime($hoadon->created_at))}}</span>
      </td>
    </tr>
    <tr>
      <td colspan="5" align="left"></td>
...
<table class="list_table" width="700">
    ...
    <!---->
        ...
        <!---->
      @foreach ($hoadon->hoadonban_cthoadonban as $items)
        <tr valign="middle">
            <td>{{$items->cthoadonban_sanpham->id}}</td>
            ...
            <td align="center">{{number_format($items->cthoadonban_sanpham->GiaKhuyenMai, 0, ',', '.')}} đ</td>
            <td align="center">{{$items->SoLuong}}</td>            
            <td>{{number_format($items->cthoadonban_sanpham->GiaKhuyenMai * $items->SoLuong, 0, ',', '.')}} đ</td>
        </tr>
      @endforeach
  ...
  <tr>
    ...
    <td style="background:#b8cce4;">{{number_format($hoadon->TongTien,0,',','.')}} đ</td>
  </tr>
</tbody></table>
