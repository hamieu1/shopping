...
<table width="700">
    ...
    <tr>
        <td colspan="8" style="border-top: 4px double #ccc;;font-size:21px; font-weight:bold; text-align:center; padding:15px 0;">BẢNG BÁO CÁO ĐƠN HÀNG</td>
    </tr>
</tbody></table>
    ...
    <!---->
      ...
      @foreach ($hdban as $hd)
        <tr valign="middle">
            ...
            <td colspan="2">{{$hd->hoadonban_khachhang->Name}}</td>
          @if ($hd->TrangThai == "Khởi tạo")
            <td>{{$hd->TrangThai}}</span></td>
          @elseif ($hd->TrangThai == "Hoàn thành")
            <td>{{$hd->TrangThai}}</span></td>
          @else 
            <td>{{$hd->TrangThai}}</span></td>
          @endif            
            <td align="center">{{number_format($hd->TongTien, 0, ',', '.')}} đ</td>
            <td align="center">{{$hd->Payment}}</td>  
        </tr>
      @endforeach
